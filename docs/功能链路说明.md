...
#### 1.1 用户输入处理
**文件位置**: `frontend/src/App.tsx` - `handleUnifiedInput` 函数
...
#### 1.2 批量翻译执行
**文件位置**: `frontend/src/App.tsx` - `handleBatchTranslate` 函数

**调用链路**:
```
前端 `handleBatchTranslate` → GoAPI.TranslateText() → 后端 `TranslateText()` → `SegmentText()`
```
...
#### 1.3 翻译结果显示
**文件位置**: `frontend/src/App.tsx` - `MixedTranslation` 组件
...
#### 2.1 词根数据加载
**文件位置**: `frontend/src/App.tsx` - `getAllRoots` 函数

**调用链路**:
```
前端 getAllRoots() → GoAPI.GetAllRoots() → 后端 app.go:68 → 数据库查询
```
...
#### 2.2 添加单个词根
**文件位置**: `frontend/src/App.tsx` - `handleAddCustomRoot` 函数
...
#### 2.3 批量导入词根
**文件位置**: `frontend/src/App.tsx` - `RootManagement` 组件
...
#### 2.4 词根编辑功能
**文件位置**: `frontend/src/App.tsx` - `handleSaveEdit` 函数
...
#### 3.1 词根库导出
**文件位置**: `frontend/src/App.tsx` - `handleExportCSV` 函数
...
#### 3.2 翻译结果导出
**文件位置**: `frontend/src/App.tsx` - `handleExportTSV` 函数
...
#### 7.2 实时搜索
**文件位置**: `frontend/src/App.tsx` - 搜索过滤逻辑

**实现**: 基于当前搜索词过滤词根列表

#### 7.3 确认对话框
**文件位置**: `frontend/src/App.tsx` - 清空确认对话框逻辑
